...
def run_full_simulation(cfg):
    ...
    for i in range(len(t_grid_s) - 1):
        ...
        # *** BUG FIX ***
        # Convert results back to the original dtype before storing them.
        ...
        # Check if any binaries merged in this timestep
        merger_this_step = a_new < 1e-20
        if merger_this_step.any():
            current_z = z_grid[i]
            merged_indices_in_active = merger_this_step.nonzero().flatten()

            # Map the indices from the 'active' array back to the original array
