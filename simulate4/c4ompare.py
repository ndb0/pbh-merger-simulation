...
elif np.any(np.abs(analytical_total_rates) > 1e15): print("\nWARNING: Analytical rates seem large.")
    ...
    for f_val in f_pbh_simulated:
        ...
        if os.path.exists(sim_file):
            try:
                ...
                # Ensure rates are positive before taking log10
                an_rates_pair = analytical_pair_rates[pair_key]
                valid_an_mask = np.isfinite(an_rates_pair) & (an_rates_pair > 0)
                if np.sum(valid_an_mask) >= 2: # Need at least 2 points for interpolation
                ...
                else: rate_an_interp = 0.0 # Cannot interpolate
...
# --- Create Plot ---
print("\nGenerating Plot...")
fig, ax = plt.subplots(figsize=(10, 8)) # Use fig, ax for clarity

# Total Analytical Rate
valid_total = np.isfinite(analytical_total_rates) & (analytical_total_rates > 0)
if np.any(valid_total): ax.loglog(f_pbh_values_plot[valid_total], analytical_total_rates[valid_total], color='white', linestyle='-', linewidth=2, label='Total Analytical Rate (Integrated)')
else: print("Warning: No valid total analytical rates.")

# Pair Rates
plot_sim_success = False
for m1_sim, m2_sim in mass_pairs_sim:
    pair_key = f"{m1_sim:.1e}-{m2_sim:.1e}"; color = colors.get(pair_key, 'dimgray'); label = labels.get(pair_key, pair_key) # Changed default color
    # Analytical Density
    ...
    if np.any(valid_sim):
        # Use plot instead of loglog if data might contain zeros or negatives after calculation
        ax.plot(sim_fs[valid_sim], sim_rates[valid_sim], color=color, linestyle='-', marker='o', markersize=6, linewidth=1.5, label=f'{label} (Simulation w/ Remapping)')
...
ax.fill_between(f_pbh_values_plot, lvk_min, lvk_max, color='dimgray', alpha=0.5, label='LVK O3 Rate Range (Approx)') # Adjusted color/alpha

# --- Customize Plot ---
ax.set_xscale('log'); ax.set_yscale('log') # Set scales on axes object
ax.set_xlim(1.e-4, 1.); ax.set_ylim(1e-2, 1e6) # Adjust ylim if needed
...
ax.legend(loc='lower right', fontsize='medium', facecolor='black', edgecolor='gray') # Added background/edge
ax.grid(True, which='both', linestyle=':', linewidth=0.5, color='gray')
fig.tight_layout()

plot_filename = "pbh_merger_rate_comparison_unequal_c4.pdf"
