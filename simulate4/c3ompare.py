...
try:
    # Use revised function name and also import sigma_eq if needed elsewhere
    from p2bh_merger_rate_callable import calculate_merger_rate_matrix, sigma_eq
except ImportError:
...
def mass_func_psi_normalized(m):
    ...
    result = pdf1 + pdf2
    # Ensure non-negative and handle arrays/scalars
...
for f_val in f_pbh_values_plot:
    print(f"  f = {f_val:.3e}:")
    try:
        # Use revised function signature and specify analytical moments
        m_out, rate_mat, _, total_rate = calculate_merger_rate_matrix(
            f_total=f_val,
            psi_interp_func=psi_interp_callable, # Pass interpolator
            mlist=analytical_mass_grid,
            m1_peak=m1_peak, # Provide peaks for analytical moments
            m2_peak=m2_peak,
            use_analytical_moments=True # Use analytical moments
        )
...
if np.all(np.isnan(analytical_total_rates)): print("\nERROR: All analytical total rates are NaN.")
elif np.nanmax(analytical_total_rates) > 1e15: print("\nWARNING: Analytical rates seem large.")
    ...
    for f_val in f_pbh_simulated:
        ...
        if os.path.exists(sim_file):
            try:
                ...
                # Interpolate analytical rate density for this pair/f
                log_f_analytical = np.log10(f_pbh_values_plot)
                log_rate_analytical_pair = np.log10(analytical_pair_rates[pair_key])
                valid_an = np.isfinite(log_rate_analytical_pair)
                if np.any(valid_an) and len(log_f_analytical[valid_an]) > 1:
                     log_rate_an_interp = np.interp(np.log10(f_val), log_f_analytical[valid_an], log_rate_analytical_pair[valid_an])
                ...
                elif frac_remapped > 1e-9: sim_rate_est = rate_an_interp * frac_remapped / 1e-6 # Arbitrary scaling
...
if np.any(valid_total): plt.loglog(f_pbh_values_plot[valid_total], analytical_total_rates[valid_total], color='white', linestyle='-', linewidth=2, label='Total Analytical Rate (Integrated)')
...
for m1_sim, m2_sim in mass_pairs_sim:
    ...
    if np.any(valid_an): plt.loglog(f_pbh_values_plot[valid_an], an_rates[valid_an], color=color, linestyle='--', linewidth=1.5, label=f'{label} (Analytical Density)')
    ...
    if np.any(valid_sim):
        plt.loglog(sim_fs[valid_sim], sim_rates[valid_sim], color=color, linestyle='-', marker='o', markersize=6, linewidth=1.5, label=f'{label} (Simulation w/ Remapping)')
...
# --- Customize Plot ---
plt.xlim(1.e-4, 1.); plt.ylim(1e-2, 1e6) # Adjust ylim if needed
plt.xlabel(r'$f_{\rm PBH}$'); plt.ylabel(r'Merger Rate [Gpc$^{-3}$ yr$^{-1}$]')
plt.title('PBH Merger Rate: Analytical vs. Simulation (Unequal Mass)')
plt.legend(loc='lower right', fontsize='medium') # Increased font size
plt.grid(True, which='both', linestyle=':', linewidth=0.5, color='gray')
plt.tight_layout()

plot_filename = "pbh_merger_rate_comparison_unequal_log_mass_interval.png"
