...
# CRITICAL USER REQUEST: Vary r_initial from r_max / 1e10 to r_max
...
# --- KERNEL REVERSION: USING FULL COSMOLOGICAL KERNEL (REMOVING PURE GRAV OVERRIDE) ---
# The physics must include the Hubble acceleration component.
...
def setup_three_body_si(r_initial, d_perturber, m1_si, m2_si, m3_si, t_start):
    """
    Sets up the initial state for the 3-body system at t_start using Hubble flow and a stabilizing angular kick.
    """
    ...
    # --- CRITICAL FIX: ADD MINIMAL TANGENTIAL VELOCITY ---

    # Calculate the local Keplerian orbital velocity V_k = sqrt(G*M/r_initial)
    V_kepler_sq = (G_SI * M_inner) / r_initial
    V_kepler = math.sqrt(V_kepler_sq) # [m/s]

    # Use 1e-6 of Keplerian velocity for stabilization (j ~ 1e-6)
    V_tangential_scale = 1e-6 * V_kepler # [m/s]

    # Calculate stabilizing velocities relative to CoM
    V1_rel_mag = (m2_si / M_inner) * V_tangential_scale
    V2_rel_mag = (m1_si / M_inner) * V_tangential_scale

    # Create the stabilizing V_peculiar (tangential in y-direction)
    V_peculiar = torch.zeros((3, 3), dtype=torch.float64)
    V_peculiar[0, 1] = V1_rel_mag
    V_peculiar[1, 1] = -V2_rel_mag
    V_peculiar[2, 1] = 0.0 # Perturber has no peculiar velocity

    velocities = V_hubble + V_peculiar # [m/s]

    # Final CoM subtraction for the 3-body system
...
def run_single_three_body_sim(r_initial, d_perturber, m1_si, m2_si, m3_si, M_binary_si, log_buffer):
    ...
    for k in range(NUM_TIMESTEPS):
        ...
        # --- CRITICAL CHANGE: Use the FULL COSMOLOGICAL KERNEL ---
