...
def rho(r, r_tr, M_PBH, gamma=3.0/2.0):
    ...
    r_eq_safe = np.clip(r_eq(M_PBH), 1e-30, None)

    A_denom = (4 * np.pi * (r_tr_safe**gamma) * (r_eq_safe**(3 - gamma)))

    # Handle potential zero denominator if inputs were bad
    # Initialize A as an array of zeros with the same shape as A_denom
...
def Menc(r, r_tr, M_PBH, gamma=3.0/2.0):
    # CRASH FIX: Vectorized implementation
    # Ensure r is an array for broadcasting
    r = np.atleast_1d(r)
    x = r / r_tr

    # Clip r_eq to prevent division by zero
    r_eq_safe = np.clip(r_eq(M_PBH), 1e-30, None)
    r_tr_safe = np.clip(r_tr, 1e-30, None) # r_tr can also be an array

    # Use np.where for vectorized conditional logic
    M_enc = np.where(
        x <= 1,
        M_PBH * (1 + (r / r_eq_safe)**(3 - gamma)),
        M_PBH * (1 + (r_tr_safe / r_eq_safe)**(3 - gamma))
    )
    return M_enc


def calcBindingEnergy(r_tr, M_PBH):
    # This function is only called inside a loop, so it's safe for r_tr to be scalar
    if r_tr <= 1e-8: return 0.0
    # We must use the scalar versions (rho_scalar, Menc_scalar) for quad
...
def getBindingEnergy(r_tr, M_PBH):
    ...
    if ((M_PBH - current_MPBH)**2 > 1e-3 or Ubind_interp is None):
        ...
        # This loop is correct, as calcBindingEnergy uses quad
    ...
    # Ubind_interp *can* handle an array input
...
def calc_af(ai, M_total):
    ...
    # CRASH FIX: Must use the vectorized Menc function here
    ...
    # getBindingEnergy uses the interpolator, which is fine with arrays
