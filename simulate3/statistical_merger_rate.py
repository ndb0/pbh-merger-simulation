...
def t_coal(a, e, M_total, eta):
    ...
    # CRASH FIX: Vectorized check for valid inputs
    if np.any(a <= 0) or np.any(e < 0) or np.any(e >= 1) or eta <= 0 or M_total <= 0:
        # Create a result array, set invalid entries to 1e99
        t = np.full_like(a, 1e99)
        valid_mask = (a > 0) & (e >= 0) & (e < 1)

        if not np.any(valid_mask): return t # All invalid
        ...
        a_valid = a
        e_valid = e
        valid_mask = slice(None) # Select all
        t = np.empty_like(a)
