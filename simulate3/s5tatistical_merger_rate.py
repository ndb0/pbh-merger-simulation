...
# These functions must be "vectorized" to handle both scalar inputs (from MCMC)
# and array inputs (from interpolation)

def r_trunc(z, M_PBH):
    r0 = 6.3e-3 #1300 AU in pc
    # Vectorized check for z > 0
...
def r_eq(M_PBH):
    return r_trunc(z_eq, M_PBH) # M_PBH is scalar, so r_eq is scalar
...
def xbar(f, M_PBH):
    ...
    if f <= 0: f = 1e-9
    # M_PBH can be an array or scalar
    M_PBH_safe = np.clip(np.atleast_1d(M_PBH), 1e-30, None)
    result = (3.0*M_PBH_safe/(4*np.pi*rho_eq*(0.85*f)))**(1.0/3.0)
    return result[0] if np.isscalar(M_PBH) else result

def bigX(x, f, M_PBH):
    # M_PBH can be an array or scalar. x is array or scalar.
    # CRASH FIX: This handles all broadcasting cases correctly.
    xb = xbar(f, M_PBH) # Can return scalar or array

    # Ensure safe division
    xb_safe = np.clip(np.atleast_1d(xb), 1e-30, None)
    x_arr = np.atleast_1d(x)

    result = (x_arr / xb_safe)**3.0

    # Return scalar if both inputs were scalar
    if np.isscalar(x) and np.isscalar(M_PBH):
        return result[0]
    # Handle array / scalar broadcasting
    if result.shape == (1,) and (np.isscalar(x) or x.shape == (1,)):
        return result[0]
    return result
...
def rho(r, r_tr, M_PBH, gamma=3.0/2.0):
    ...
    r = np.clip(np.atleast_1d(r), 1e-30, None)
    r_tr = np.clip(np.atleast_1d(r_tr), 1e-30, None)
    x = r / r_tr

    r_eq_sc = r_eq(M_PBH) # r_eq is scalar
    if r_eq_sc <= 0: r_eq_sc = 1e-30

    A_denom = (4 * np.pi * (r_tr**gamma) * (r_eq_sc**(3 - gamma)))
    ...
    return rho_val[0] if (np.isscalar(r) and np.isscalar(r_tr)) else rho_val

def Menc(r, r_tr, M_PBH, gamma=3.0/2.0):
    # Vectorized implementation
    is_scalar_r = np.isscalar(r)
    is_scalar_r_tr = np.isscalar(r_tr)

    r_arr = np.clip(np.atleast_1d(r), 1e-30, None)
    r_tr_arr = np.clip(np.atleast_1d(r_tr), 1e-30, None)
    x = r_arr / r_tr_arr
    ...
    M_enc = np.where(
        x <= 1,
        M_PBH * (1 + (r_arr / r_eq_sc)**(3 - gamma)),
        M_PBH * (1 + (r_tr_arr / r_eq_sc)**(3 - gamma))
    )
    return M_enc[0] if (is_scalar_r and is_scalar_r_tr) else M_enc
...
def P_j(j, x, f, M_PBH):
    ...
    is_scalar_jX = np.isscalar(jX)
    jX_arr = np.atleast_1d(jX)
    jX_arr = np.clip(jX_arr, 1e-30, None) # Prevent divide by zero

    y = j/jX_arr
    prob = (y**2/(1+y**2)**(3.0/2.0))/j

    if j <= 0: return 0.0

    prob[jX_arr <= 0] = 0.0

    return prob[0] if is_scalar_jX else prob
...
def P_a_j_withHalo(a, j, f, M_PBH):
    ...
    X = bigX(xval, f, M_PBH)
    xb = xbar(f, M_PBH)
    if (np.isscalar(xb) and xb <= 0) or (not np.isscalar(xb) and np.any(xb <= 0)):
        return 0.0
    ...
    if np.isscalar(prob_j):
        ...
        prob_j = np.clip(prob_j, 1e-300, None) # Set floor instead of 0
