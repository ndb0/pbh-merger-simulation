...
#
# !! FINAL VERSION: Corrects NameError, SynatxError, and improves sampling for EMRIs !!
...
h = 0.678; Omega_DM = 0.1186/(h**2); H0 = 100.0*h
...
# --- Formation Physics Helpers (ADDED BACK FOR INTERPOLATOR SETUP) ---
...
# --- Halo Properties (Interpolation Dependencies) ---
def r_trunc(z, M_PBH):
...
def lnprob(theta, f, M1, M2, M_avg_psi, PDF_func_interp, a1, a2, tmin, tmax):
    ...
    if not (np.isfinite(la) and np.isfinite(lj)): return -np.inf
    try:
        pdf_val = PDF_func_interp(la, lj)[0, 0]
    except Exception:
        pdf_val = 0.0
...
def GetSamples_MCMC(N_samps, PDF_func_interp, a1, a2, f, M1, M2, M_avg_psi,
    ...
    total_steps = samples_per_walker * thin_by + burn_in_steps

    a0 = np.sqrt(max(a1, 1e-9) * a2) if a1 > 0 and a2 > 0 else 1e-3
    t0_guess = np.sqrt(tmin_sampling * tmax_sampling) 
    if t0_guess < tmin_sampling or t0_guess > tmax_sampling: t0_guess = tmin_sampling
    
    j0 = j_coal(a0, t0_guess, M1=M1, M2=M2)
    if j0 <= 1e-9 or j0 >= 1.0: j0 = 1e-3
    j0 = max(1e-9, min(j0, 0.99999))

    p0 = []; attempts = 0; max_attempts = nwalkers * 50000 # Increased attempts
    ...
    while len(p0) < nwalkers and attempts < max_attempts:
        attempts += 1
        la_start = np.random.uniform(np.log10(a1), np.log10(a2))
        lj_start = np.random.uniform(np.log10(j0) - 2, np.log10(j0) + 2)
        lj_start = max(np.log10(1e-9), min(lj_start, np.log10(0.99999)))
...
Nsamples_per_run = 4096
amin_glob = 5.e-5
# *** FIX: Widen sampling window for EMRIs ***
tmin_sampling = 1.e-10 # Start from 1000 years
tmax_sampling = 1.e11 # Go up to 100 Gyr

# === MODIFIED: Only run the unequal mass pair ===
mass_pairs = [
    (1.0, 1e-4) # M1=1, M2=1e-4
]
# ===============================================
...
for m1, m2 in mass_pairs:
    ...
    for f_val in f_pbh_values:
        ...
        try:
            ...
            def PDF_unequal_mass_interp(la, lj):
                try:
        ...
        try:
            ...
            # Pass a lambda function to lnprob that captures the interpolator
            pdf_lambda = lambda la, lj, f, M1, M2, M_avg_psi: PDF_unequal_mass_interp(la, lj)

            samples_MCMC = GetSamples_MCMC(Nsamples_per_run, pdf_lambda, # Pass lambda
...
if 'pair_key' in locals() and 'f_key' in locals():
    ...
    if 'samples_a_initial' in results.get(last_pair_key, {}).get(last_f_key, {}):
        ...
        if len(t_init_finite) > 0 or len(t_remap_finite) > 0:
            ...
            if len(all_finite) > 0:
                t_min_plot = max(1e3, np.min(all_finite)); t_max_plot = min(1e15, np.max(all_finite))
