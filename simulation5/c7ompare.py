# compare.py
# Corrected dictionary keys for colors/labels
...
colors = {'1.0e+00-1.0e+00': 'red',       # M1-M1 pair
          '1.0e-04-1.0e-04': 'blue',      # M2-M2 pair
          '1.0e+00-1.0e-04': 'green'}     # M1-M2 pair
...
analytical_pair_rates = {f"{m1:.1e}-{m2:.1e}": [] for m1, m2 in mass_pairs_sim} # Use correct format for keys here too
analytical_mass_grid = np.logspace(np.log10(m_min_psi), np.log10(m_max_psi), 200)

for f_val in f_pbh_values_plot:
    # print(f"  f = {f_val:.3e}:") # Reduce print clutter
    try:
        ...
        for m1_sim, m2_sim in mass_pairs_sim:
            ...
            pair_key = f"{m1_sim:.1e}-{m2_sim:.1e}" # Generate key consistent with dict
            rate_val = rate_mat[idx1, idx2]
            if not np.isfinite(rate_val) or abs(rate_val) > 1e20: rate_val = np.nan
            # Check if key exists before appending (should always exist if mass_pairs_sim matches)
            if pair_key in analytical_pair_rates:
                analytical_pair_rates[pair_key].append(rate_val)
            else:
                print(f"Warning: Generated pair_key '{pair_key}' not found in dictionary during analytical calculation.")
...
for m1_sim, m2_sim in mass_pairs_sim:
    pair_key = f"{m1_sim:.1e}-{m2_sim:.1e}"; print(f"  Pair {pair_key}:") # Use corrected key format
    for f_val in f_pbh_simulated:
        f_key = f"f_{f_val:.3e}"
        # Construct filename consistent with how unequal.py saves it
        sim_file_pair_key = f"M1_{m1_sim:.1e}_M2_{m2_sim:.1e}" # Match unequal.py format
        ...
        if os.path.exists(sim_file):
            try:
                ...
                if pair_key in analytical_pair_rates:
                    ...
                    print(f"      Error: Key '{pair_key}' not found in analytical_pair_rates.")
                    continue # Skip this f_val if key is missing
...
for m1_sim, m2_sim in mass_pairs_sim:
    pair_key = f"{m1_sim:.1e}-{m2_sim:.1e}"; # Generate key consistent with dict
    # Get color and label using the CORRECTED key
    color = colors.get(pair_key, 'dimgray'); label = labels.get(pair_key, pair_key)
...
ax.fill_between(f_pbh_values_plot, lvk_min, lvk_max, color='gray', alpha=0.4, label='LVK O3 Rate Range (Approx)', zorder=1)
