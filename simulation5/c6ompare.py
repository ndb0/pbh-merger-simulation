...
# === MODIFIED COLORS ===
colors = {'1.0-1.0': 'red',              # M1-M1 pair in Red
          '1.0e-04-1.0e-04': 'blue',     # M2-M2 pair in Blue
          '1.0-1.0e-04': 'green'}        # M1-M2 pair in Green
# ========================
...
ax.fill_between(f_pbh_values_plot, lvk_min, lvk_max, color='lightgray', alpha=0.6, label='LVK O3 Rate Range (Approx)', zorder=1) # Adjusted alpha

# --- Customize Plot ---
ax.set_xscale('log'); ax.set_yscale('log')
ax.set_xlim(1.e-5, 1.); ax.set_ylim(1e-6, 1e6) # Adjust ylim if needed
...
plt.savefig(plot_filename_pdf, dpi=300, format='pdf') # Specify format
print(f"\nComparison plot saved to {plot_filename_pdf}")
# ===================

print("\nPlotting Script Finished.")

# Optional: Close the plot if running non-interactively
plt.close(fig)
