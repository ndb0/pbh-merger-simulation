...
# Uses standard plotting style and saves as PDF.
...
try:
    from p8bh_merger_rate_callable import calculate_merger_rate_matrix, sigma_eq
except ImportError:
...
ax_psi.loglog(mass_grid_psi, psi_vals_grid, color='blue', linewidth=2) # Changed color
ax_psi.axvline(m1_peak, color='red', linestyle='--', alpha=0.7, label=f'$M_1$ Peak ({m1_peak}$M_\\odot$)')
ax_psi.axvline(m2_peak, color='purple', linestyle='--', alpha=0.7, label=f'$M_2$ Peak ({m2_peak}$M_\\odot$)') # Changed color
...
max_psi_val = np.max(psi_vals_grid)
ymin_psi = max(1e-9, min_psi_val / 10)
ymax_psi = max_psi_val * 10
...
ax_psi.grid(True, which='both', linestyle=':', linewidth=0.6, color='lightgray')
fig_psi.tight_layout()
psi_plot_filename = "psi_mass_function8.png"
...
analytical_mass_grid = np.logspace(np.log10(m_min_psi), np.log10(m_max_psi), 150)
...
for m1_sim, m2_sim in mass_pairs_sim:
    pair_key = f"{m1_sim:.1e}-{m2_sim:.1e}"
    color = colors.get(pair_key, 'gray'); label = labels.get(pair_key, pair_key)
    # Analytical Density
    if pair_key in analytical_pair_rates:
...
plot_filename_pdf = "pbh_merger_rate_comparison_unequal_whiteBG_c8.pdf"
