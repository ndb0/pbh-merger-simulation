...
# using analytical moments and CPU parallelization via joblib.
...
import warnings
import os
import multiprocessing # Added for core count
...
def mass_func_psi_normalized(m):
    ...
    # FIX: Corrected typo mm2_peak to m2_peak
...
def calculate_analytical_moments():
    ...
    # <m> = 1 / integral(0.5/m1 + 0.5/m2) dm
    # For sharp peaks, integral(psi/m dm) approx 0.5/m1 + 0.5/m2 (sum of delta functions)
    denom_avM = 0.5/m1_peak + 0.5/m2_peak
    avM = 1.0 / denom_avM if denom_avM > 0 else 1.0
    # <m^2> approx 0.5*m1^2 + 0.5*m2^2
...
def calculate_pdf_slice(la_slice, lj_grid_vals, f_pbh, M1, M2, M_avg, sigma_M, N_y):
    """Calculates a slice of the PDF grid for a given la range."""
    N_a_slice = len(la_slice)
    N_j = len(lj_grid_vals)
    pdf_slice = np.zeros((N_a_slice, N_j))

    # Helper for inner integral over mass (m)
    ...
    for i, la in enumerate(la_slice):
        a = 10**la
        X = calculate_X_from_a(a, f_pbh, M_avg)
        P_a_factor = np.exp(-X) * (a**(-0.25)) if a > 0 else 0.0
        if not (np.isfinite(P_a_factor) and P_a_factor > 0): continue
        ...
        for k, lj in enumerate(lj_grid_vals):
            ...
            # Integrand for the outer integral (over u)
            def integrand_u(u):
                if u < 0: return 0.0
                try:
            ...
            # Perform the outer integral over u
            try:
                with warnings.catch_warnings():
                    warnings.simplefilter("ignore")
            ...
            pdf_slice[i, k] = P_a_j_unnorm * a * j * (np.log(10)**2)
...
def precompute_and_save_grid(M1, M2, f_pbh, 
    ...
    # Split the la_grid into chunks for parallel processing
    la_chunks = np.array_split(la_grid_vals, multiprocessing.cpu_count() if n_jobs == -1 else n_jobs)

    # Run the calculation in parallel
    results_list = Parallel(n_jobs=n_jobs)(
        delayed(calculate_pdf_slice)(la_slice, lj_grid_vals, f_pbh, M1, M2, M_avg, sigma_M, N_y)
        for la_slice in la_chunks
    )

    # Recombine results
    pdf_grid = np.concatenate(results_list, axis=0)
...
if __name__ == "__main__":
    ...
    # --- Parameters for the grid you want to compute ---
    M1_run = 1.0
    M2_run = 1e-4
    f_pbh_run = 0.01
    
    # Grid resolution (50x50 is a coarse test. 150x150 is often sufficient for publication)
    N_a_grid = 150 
    N_j_grid = 150 
    ...
    # Run the precomputation (will use all available cores)
    precompute_and_save_grid(M1_run, M2_run, f_pbh_run,
                             N_a_grid, N_j_grid,
                             a_min_grid, a_max_grid,
                             j_min_grid, j_max_grid,
                             n_jobs=-1)
