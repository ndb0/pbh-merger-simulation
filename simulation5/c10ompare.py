...
# REVISED: Precomputes moments and disrupting fraction matrix ONCE.
...
try:
    from p10bh_merger_rate_callable import calculate_merger_rate_matrix, sigma_eq, calculate_disrupting_fraction
except ImportError:
...
# ... (Keep psi definition, grid, normalization check, interpolator creation) ...
...
try: psi_interp_callable = scipy.interpolate.interp1d(unique_m_psi, psi_vals_grid[unique_idx_psi], bounds_error=False, fill_value=0.0, kind='linear')
...
psi_plot_filename = "psi_mass_function10.png"
...
# Use a consistent grid for rate matrix calculation across f values
analytical_mass_grid = np.logspace(np.log10(m_min_psi), np.log10(m_max_psi), 250) # Use 150 points

# === PRECOMPUTATION BLOCK ===
print("\nPrecomputing moments and disrupting fraction matrix...")
precomp_start_time = time.time()
use_analytical_moments_precomp = True # Choose method for moments
...
# Precompute Disrupting Fraction Matrix (once)
N_grid = len(analytical_mass_grid)
disrupt_fraction_matrix_precomp = np.ones((N_grid, N_grid))
print(f"  Precomputing {N_grid}x{N_grid} disrupting fraction matrix...")
for i in range(N_grid):
    for j in range(N_grid):
        mi = analytical_mass_grid[i]; mj = analytical_mass_grid[j]
        if mi > 0 and mj > 0:
            # Pass full grid used for moment calculation if needed by integrate range
            disrupt_fraction_matrix_precomp[i, j] = calculate_disrupting_fraction(
                mi, mj, psi_interp_callable, analytical_mass_grid, avM_precomp
            )
...
analytical_start_time = time.time() # Start timing loop
...
for f_val in f_pbh_values_plot:
    ...
    try:
        ...
        m_out, rate_mat, _, total_rate = calculate_merger_rate_matrix(
            ...
            avM=avM_precomp, # Pass precomputed
            avM2=avM2_precomp, # Pass precomputed
            disrupt_fraction_matrix=disrupt_fraction_matrix_precomp # Pass precomputed
        )
        # ==============================
        ...
        # print("    Pair Rates (dR/dlnm1 dlnm2):") # Reduce clutter
    ...
    # f_end_time = time.time() # Reduce print clutter
...
ax.set_xlim(1.e-5, 1.); ax.set_ylim(1e-6, 1e10) # Adjusted ymin to see suppressed rates
...
plot_filename_pdf = "pbh_merger_rate_comparison_unequal_whiteBG_c10.pdf" # New name
