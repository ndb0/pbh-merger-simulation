...
# Uses scipy.integrate.quad for 1D integrals (moments if numerical, disrupting frac).
# Accepts precomputed moments and disrupting fraction matrix.

import numpy as np
import scipy.interpolate
import scipy.integrate # Use quad
...
# --- Function to Calculate Disrupting Fraction (Helper - USES QUAD) ---
def calculate_disrupting_fraction(m1, m2, psi_interp_func, m_min_grid, m_max_grid, avg_mass):
    """
    ...
    # Define the integrand psi(m)/m
    integrand_disrupt = lambda m: psi_interp_func(m) / m if m > 0 else 0.0

    # Integrate from m_threshold up to the maximum mass considered in psi
    integration_limit_upper = m_max_grid # Use the upper limit of the grid where psi is defined
    integration_limit_lower = max(m_threshold, m_min_grid) # Ensure lower limit is within grid

    integral_disrupt = 0.0
    # Only integrate if the range is valid
    if integration_limit_upper > integration_limit_lower:
        try:
            # Use quad for potentially better accuracy with sharp peaks
            integral_disrupt, abserr = scipy.integrate.quad(
                integrand_disrupt,
                integration_limit_lower,
                integration_limit_upper,
                limit=150, # Adjusted limit
                epsabs=1e-10, # Tighter tolerance
                epsrel=1e-10
            )
            integral_disrupt = max(0.0, integral_disrupt) # Ensure non-negative
        except Exception as e:
            # print(f"Warning: quad integration failed in calculate_disrupting_fraction (m1={m1:.1e}, m2={m2:.1e}): {e}")
            integral_disrupt = 0.0 # Assign 0 on failure
...
def S_E(mi, mj, fpbh0_tot, avM, avM2, sigma_eq_val, disrupting_fraction):
    ...
    # Use np.exp with check for large negative exponent leading to zero
    exp_arg = -nnn_eff_val
    t3_factor = np.exp(exp_arg) if exp_arg > -700 else 0.0 # Avoid underflow warning/zero
...
def calculate_merger_rate_matrix(f_total, psi_interp_func, mlist, # Original args
                                  m_min_psi, m_max_psi, # Pass grid limits for quad
    ...
    # print(f"    Using precomputed <m> = {avM:.3g}, <m^2> = {avM2:.3g}") # Reduce print
...
if __name__ == '__main__':
    ...
    # Precompute Disrupt Fraction Matrix using quad
    test_mlist = np.logspace(-5, 2, 50)
    N_test = len(test_mlist)
    disrupt_frac_matrix_ex = np.ones((N_test, N_test))
    print("Precomputing disrupt fraction matrix for example (using quad)...")
    for i in range(N_test):
        for j in range(N_test):
            mi = test_mlist[i]; mj = test_mlist[j]
            if mi > 0 and mj > 0:
                disrupt_frac_matrix_ex[i, j] = calculate_disrupting_fraction(
                    mi, mj, psi_interp_ex, m_min_psi_ex, m_max_psi_ex, avM_ex # Pass quad limits
                )
    ...
    mass_out, rate_mat_out, fpbh0_out, total_rate_out = calculate_merger_rate_matrix(
        ...
        mlist=test_mlist,
        m_min_psi=m_min_psi_ex, m_max_psi=m_max_psi_ex, # Pass grid limits
