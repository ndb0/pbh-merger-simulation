...
# NEW: Loops through all f_pbh values automatically.
...
def precompute_and_save_grid(M1, M2, f_pbh, 
    ...
    la_chunks = np.array_split(la_grid_vals, n_cores)

    results_list = Parallel(n_jobs=n_jobs, verbose=10)(
        delayed(calculate_pdf_row)(la_slice, lj_grid_vals, f_pbh, M1, M2, M_avg, sigma_M, N_y)
...
if __name__ == "__main__":
    ...
    M1_run = 1.0
    M2_run = 1e-4
    # This list must match the f_pbh_values in unequal.py
    ...
    # --- Loop over f_pbh and precompute all necessary files ---
    print(f"Starting batch precomputation for {len(f_pbh_values_to_compute)} f_pbh values...")
    for f_val in f_pbh_values_to_compute:
        precompute_and_save_grid(M1_run, M2_run, f_val,
                                 N_a_grid, N_j_grid,
                                 a_min_grid, a_max_grid,
                                 j_min_grid, j_max_grid,
                                 n_jobs=-1)
