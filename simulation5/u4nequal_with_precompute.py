...
# !! SYNTAX FIX in M_halo function !!
# !! MODIFIED to ONLY run the M1=1.0, M2=1.e-4 pair !!
...
def M_halo(z, M_PBH):
    if M_PBH <= 0: return 0.0
    # --- SYNTAX FIX: Moved 'if' to new line ---
    req = r_eq(M_PBH)
    if req <= 0: return 0.0
    # --- END FIX ---
...
def a_max_generalized(f, M_avg_psi, withHalo=False, M_PBH_for_halo=1.0):
     global alpha, lambda_max, z_eq, rho_eq
     if alpha <= 0 or f <= 0: return 0.0
     M_eff = M_avg_psi
     if withHalo: M_eff += M_halo(z_eq, M_PBH_for_halo) 
     xb = xbar_calc(f, M_eff)
     if xb <= 0: return 0.0
     term_f = f * 0.85
     if term_f < 0: return 0.0
     return alpha * xb * np.power(term_f, 1.0/3.0) * np.power(lambda_max, 4.0/3.0)
def a_max_with_Halo_generalized(f, M_PBH_for_halo, M_avg_psi):
     return a_max_generalized(f, M_avg_psi, withHalo=True, M_PBH_for_halo=M_PBH_for_halo)
...
def lnprob(theta, f, M1, M2, M_avg_psi, PDF_func_interp, a1, a2, tmin, tmax):
    ...
    # PDF_func_interp is the RectBivariateSpline interpolator
    try:
        pdf_val = PDF_func_interp(la, lj)[0, 0]
    except Exception:
        pdf_val = 0.0 # Failed interpolation
...
def GetSamples_MCMC(N_samps, PDF_func_interp, a1, a2, f, M1, M2, M_avg_psi,
    ...
    a0 = np.sqrt(max(a1, 1e-9) * a2) if a1 > 0 and a2 > 0 else 1e-3; t0_guess = ageUniverse
...
for m1, m2 in mass_pairs:
    ...
    for f_val in f_pbh_values:
        ...
        try:
            ...
            def PDF_unequal_mass_interp(la, lj, f, M1, M2, M_avg_psi):
                try:
        ...
        try:
            ...
            samples_MCMC = GetSamples_MCMC(Nsamples_per_run, PDF_unequal_mass_interp,
