...
# FIX: Corrected typo in mass function.
# FIX: Using robust analytical moments to fix 1e18 M_sun error.
# IMPLEMENTED: CPU Parallelization via joblib for speed.
...
rho_eq = 1512.0
alpha = 0.1
sigma_eq_val = np.sqrt(0.005) # This is sigma_M in Raidal et al.
z_eq = 3375.0
a_eq = 1.0 / (1.0 + z_eq)
...
def mass_func_psi_normalized(m):
    ...
    # FIX: Corrected 'mm2_peak' to 'm2_peak'
...
def calculate_analytical_moments():
    ...
    This is fast, robust, and avoids the numerical integration errors.
    """
    # <m> = 1 / integral(psi/m dm) approx 1 / (0.5/m1 + 0.5/m2)
    denom_avM = 0.5/m1_peak + 0.5/m2_peak
    avM = 1.0 / denom_avM if denom_avM > 0 else 1.0
    # <m^2> = integral(m^2 psi dm) approx 0.5*m1^2 + 0.5*m2^2
...
# This function calculates one row (one 'a' value) of the grid.
...
def precompute_and_save_grid(M1, M2, f_pbh, 
    ...
    # This maps the function 'calculate_pdf_row' to each 'la' value in 'la_grid_vals'
    results_list = Parallel(n_jobs=n_jobs, verbose=10)(
        delayed(calculate_pdf_row)(la, lj_grid_vals, f_pbh, M1, M2, M_avg, sigma_M, N_y)
        for la in la_grid_vals
    )

    # Recombine results from the list of rows into a 2D grid
...
if __name__ == "__main__":
    ...
    # Grid resolution (150x150 is the size from your log)
    N_a_grid = 150 
    N_j_grid = 150 
    ...
    print("\nPrecomputation complete. Run this for all (M1, M2, f) combinations needed.")
