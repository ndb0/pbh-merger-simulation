...
# REVISED: Accepts precomputed moments and disrupting fraction matrix.
...
# --- Function to Calculate Disrupting Fraction (Helper - still needed for precomputation) ---
def calculate_disrupting_fraction(m1, m2, psi_interp_func, m_grid, avg_mass):
    ...
    if len(m_grid_disrupt) < 2:
        ...
        # Evaluate integrand only on the relevant subgrid
    ...
    if not (np.isfinite(avg_mass) and avg_mass > 0): return 1.0
...
# ... (Keep NNN_effective, S_L, CCC, S_E as in the previous version) ...
...
def rate_density(mi, mj, f_total, psi_interp, avM, avM2, sigma_eq_val, disrupting_fraction):
    ...
    except ValueError: psi_mi = 0.0; psi_mj = 0.0 # Handle potential interp errors
...
# === MODIFIED SIGNATURE: Takes precomputed moments and disrupt_fraction_matrix ===
def calculate_merger_rate_matrix(f_total, psi_interp_func, mlist, # Original args
    ...
    """
    global sigma_eq
    # print(f"Calculating analytical rate matrix for f_PBH = {f_total:.3e}...") # Reduce print clutter
    ...
    # print(f"    Using precomputed <m> = {avM:.3g}, <m^2> = {avM2:.3g}") # Reduce clutter

    fpbh0_for_suppression = f_total; sigma_eq_val = sigma_eq

    # --- Calculate Rate Matrix using precomputed disrupt_fraction_matrix ---
    # print("    Calculating rate matrix elements...") # Reduce print clutter
    ...
    # print(f"    ... Rate matrix calculation done ({end_time_matrix - start_time_matrix:.2f} s).") # Reduce clutter
    ...
    # else: print("Warning: <2 valid mass points for total rate integration.") # Reduce clutter

    if not np.isfinite(total_rate) or total_rate < 0: total_rate = 0.0
    # elif total_rate > 1e20: print(f"Warning: Total rate very large ({total_rate:.3g}).") # Reduce clutter

    # print(f"... Analytical calculation done. Total Rate = {total_rate:.3g} Gpc^-3 yr^-1") # Reduce clutter
    fpbh0_to_return = f_total
    return mlist, rate_matrix, fpbh0_to_return, total_rate

# --- Main execution block (Example Usage) ---
if __name__ == '__main__':
    # ... (Keep example usage, but it will need to precompute moments and
    #      disrupt_fraction_matrix itself before calling the main function) ...
    import scipy.stats
    print("\nRunning pbh_merger_rate_callable.py example (with precomputation)...")

    # Define psi, grid, interpolator for example
    m1_peak_ex = 1.0; m2_peak_ex = 1.0e-4; sigma_rel_ex = 0.01
    # ... (psi definition, grid, norm check, interpolator creation as before) ...
    ...
    num_points_psi_ex = 1000 # Use a reasonable grid for moments
    ...
    # --- Precompute Moments for Example ---
    use_analytical = True # Example uses analytical moments
    if use_analytical:
        denom_avM_ex = 0.5/m1_peak_ex + 0.5/m2_peak_ex
        avM_ex = 1.0 / denom_avM_ex
        avM2_ex = 0.5 * m1_peak_ex**2 + 0.5 * m2_peak_ex**2
    else: # Numerical moments
        # ... (numerical moment calculation logic using trapz) ...
        valid_mask_ex = mass_grid_psi_ex > 0
        psi_on_mlist_ex = psi_interp_ex(mass_grid_psi_ex) # Evaluate on grid
        integrand_inv_m_ex = np.zeros_like(mass_grid_psi_ex)
        integrand_inv_m_ex[valid_mask_ex] = psi_on_mlist_ex[valid_mask_ex] / mass_grid_psi_ex[valid_mask_ex]
        integral_inv_m_ex = scipy.integrate.trapz(integrand_inv_m_ex[valid_mask_ex], mass_grid_psi_ex[valid_mask_ex])
        avM_ex = 1.0 / integral_inv_m_ex
        integrand_m2_ex = mass_grid_psi_ex**2 * psi_on_mlist_ex
        avM2_ex = scipy.integrate.trapz(integrand_m2_ex[valid_mask_ex], mass_grid_psi_ex[valid_mask_ex])

    print(f"Example moments: <m> = {avM_ex:.3g}, <m^2> = {avM2_ex:.3g}")

    # --- Precompute Disrupt Fraction Matrix for Example ---
    test_mlist = np.logspace(-5, 2, 50) # Output grid
    N_test = len(test_mlist)
    disrupt_frac_matrix_ex = np.ones((N_test, N_test))
    print("Precomputing disrupt fraction matrix for example...")
    for i in range(N_test):
        for j in range(N_test):
            mi = test_mlist[i]; mj = test_mlist[j]
            if mi > 0 and mj > 0:
                disrupt_frac_matrix_ex[i, j] = calculate_disrupting_fraction(
                    mi, mj, psi_interp_ex, mass_grid_psi_ex, avM_ex # Use example interpolator, grid, avM
                )
    ...
    mass_out, rate_mat_out, fpbh0_out, total_rate_out = calculate_merger_rate_matrix(
        ...
        mlist=test_mlist,
        avM=avM_ex, avM2=avM2_ex, # Pass precomputed moments
        disrupt_fraction_matrix=disrupt_frac_matrix_ex # Pass precomputed matrix
    )
