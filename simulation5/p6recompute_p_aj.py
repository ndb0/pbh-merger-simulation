...
#
# !! MODIFICATION: Added logic in main block to use a DIFFERENT a_range
# !! for the (1e-4, 1e-4) pair to fix the physical inconsistency.
...
def precompute_and_save_grid(M1, M2, f_pbh, 
    ...
    print(f"   a_range: [{a_min_grid:.1e}, {a_max_grid:.1e}]")
    print(f"   j_range: [{j_min_grid:.1e}, {j_max_grid:.1e}]")
...
if __name__ == "__main__":
    ...
    # ========================================================================
    # === START: MODIFIED a-range logic ===
    # ========================================================================
    
    # Default a-range (from original file)
    a_min_default = 5e-5
    a_max_default = 0.1
    
    # "Micro" a-range for the tiny mass pair
    a_min_micro = 1e-9
    a_max_micro = 1e-5
    
    # j-range (keep the same)
    j_min_grid = 1e-6
    j_max_grid = 1.0
    ...
    for M1, M2 in mass_pairs_to_compute:
        ...
        # --- Select the correct a_range based on the mass pair ---
        if np.isclose(M1, 1e-4) and np.isclose(M2, 1e-4):
            print(f"\n*** Using MICRO a-range for ({M1:.1e}, {M2:.1e}) pair ***")
            a_min_for_pair = a_min_micro
            a_max_for_pair = a_max_micro
        else:
            print(f"\n*** Using DEFAULT a-range for ({M1:.1e}, {M2:.1e}) pair ***")
            a_min_for_pair = a_min_default
            a_max_for_pair = a_max_default
        # --------------------------------------------------------
        ...
        for f_val in f_pbh_values_to_compute:
            ...
            precompute_and_save_grid(M1, M2, f_val,
                                     N_a_grid, N_j_grid,
                                     a_min_for_pair, a_max_for_pair, # Pass the selected range
                                     j_min_grid, j_max_grid,
                                     n_jobs=-1)
    # ========================================================================
    # === END: MODIFIED a-range logic ===
    # ========================================================================
