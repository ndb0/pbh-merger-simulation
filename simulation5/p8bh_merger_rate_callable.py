...
# REVISED: Implemented mass-ratio dependent disruption suppression in S_E
...
# --- Function to Calculate Disrupting Fraction ---
def calculate_disrupting_fraction(m1, m2, psi_interp_func, m_grid, avg_mass):
    """
    Calculates the fraction of the number density integral corresponding to
    PBHs massive enough to disrupt the (m1, m2) binary.
    Uses the condition m3 >= max(m1, m2).
    """
    m_threshold = max(m1, m2)

    # Numerically integrate psi(m)/m from m_threshold to max mass in grid
    integrand_disrupt = lambda m: psi_interp_func(m) / m if m > 0 else 0.0

    # Integrate only over the grid range where m >= m_threshold
    m_grid_disrupt = m_grid[m_grid >= m_threshold]
    if len(m_grid_disrupt) < 2:
        # If threshold is beyond grid, or only one point, fraction is effectively zero
        integral_disrupt = 0.0
    else:
        psi_vals_disrupt = np.array([integrand_disrupt(m) for m in m_grid_disrupt])
        integral_disrupt = scipy.integrate.trapz(psi_vals_disrupt, m_grid_disrupt)
        integral_disrupt = max(0.0, integral_disrupt) # Ensure non-negative

    # F_disrupt = <m> * integral_{disrupting} (psi(m)/m) dm
    # Ensure avg_mass is valid
    if not (np.isfinite(avg_mass) and avg_mass > 0):
        print("Warning: Invalid avg_mass in calculate_disrupting_fraction.")
        return 1.0 # Fallback: assume all masses disrupt if avg_mass is invalid
...
# --- Helper Functions for Suppression Factors (NNN modified) ---
def NNN_effective(mi, mj, avM, fpbh0_tot, sigma_eq_val, disrupting_fraction):
    ...
    # Calculate original N(y) based on total mass and average mass
    ...
    # Scale by the disrupting fraction
...
# CCC remains the same (depends on moments, f_total, sigma_eq)
...
# S_E uses NNN_effective now
def S_E(mi, mj, fpbh0_tot, avM, avM2, sigma_eq_val, disrupting_fraction):
    """Suppression factor S_E using effective NNN."""
    if not (np.isfinite(avM) and avM > 0 and np.isfinite(avM2) and avM2 >= 0 and
            np.isfinite(fpbh0_tot) and fpbh0_tot > 0 and np.isfinite(sigma_eq_val) and sigma_eq_val > 0): return 0.0
...
# --- Rate Density Function (uses new S_E) ---
def rate_density(mi, mj, f_total, psi_interp, avM, avM2, sigma_eq_val, disrupting_fraction):
    ...
    sl_val = S_L(f_total)
    # Pass disrupting_fraction to S_E
    ...
    rate_floor = 1e-99 # Keep floor for plotting
...
def calculate_merger_rate_matrix(f_total, psi_interp_func, mlist, # Use interpolator directly
    ...
    Includes mass-ratio dependent disruption suppression.
    """
    global sigma_eq
    ...
    if len(mlist_valid) < 2: return mlist, np.zeros((len(mlist), len(mlist))), f_total, 0.0

    # --- Calculate Moments ---
    # (Same logic as before - use analytical or numerical)
    ...
    if use_analytical_moments and m1_peak is not None and m2_peak is not None:
        ...
        # ... (numerical moment calculation using trapz) ...
    ...
    for i in range(len(mlist)):
        for j in range(len(mlist)):
            ...
            # Calculate disrupting fraction for this specific pair
            disrupt_frac = calculate_disrupting_fraction(mi, mj, psi_interp_func, mlist, avM)
            # Pass disrupting fraction to rate_density
...
if __name__ == '__main__':
    # ... (Example usage remains the same, ensure psi_interp_ex is defined) ...
    import scipy.stats
    ...
    test_f_total = 0.001; test_mlist = np.logspace(-5, 2, 50)
    mass_out, rate_mat_out, fpbh0_out, total_rate_out = calculate_merger_rate_matrix(
        f_total=test_f_total, psi_interp_func=psi_interp_ex, mlist=test_mlist,
        m1_peak=m1_peak_ex, m2_peak=m2_peak_ex, use_analytical_moments=True
    )
