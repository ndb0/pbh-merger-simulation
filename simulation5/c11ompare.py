...
# Uses standard plotting style and saves as PDF. Precomputes moments/disrupt frac.
...
try:
    from p11bh_merger_rate_callable import calculate_merger_rate_matrix, sigma_eq, calculate_disrupting_fraction
except ImportError:
...
analytical_mass_grid = np.logspace(np.log10(m_min_psi), np.log10(m_max_psi), 250) # Use 150 points for speed

# === PRECOMPUTATION BLOCK ===
print("\nPrecomputing moments and disrupting fraction matrix...")
precomp_start_time = time.time()
use_analytical_moments_precomp = True # Use analytical moments for sharp peaks
...
if use_analytical_moments_precomp:
    ...
    avM_precomp = 1.0 / denom_avM
    avM2_precomp = 0.5 * m1_peak**2 + 0.5 * m2_peak**2
else:
    print("  Calculating moments numerically using QUAD...")
    integrand_inv_m = lambda m: psi_interp_callable(m) / m if m > 0 else 0.0
    try: integral_inv_m, _ = scipy.integrate.quad(integrand_inv_m, m_min_psi, m_max_psi, limit=200, epsabs=1e-10)
    except Exception as e: raise ValueError(f"<m> quad failed: {e}")
    if integral_inv_m <= 1e-99: raise ValueError("<m> calc failed (integral <= 0).")
    avM_precomp = 1.0 / integral_inv_m
    integrand_m2 = lambda m: m**2 * psi_interp_callable(m) if m > 0 else 0.0
    try: avM2_precomp, _ = scipy.integrate.quad(integrand_m2, m_min_psi, m_max_psi, limit=200, epsabs=1e-10)
    except Exception as e: raise ValueError(f"<m^2> quad failed: {e}")
    if not np.isfinite(avM2_precomp) or avM2_precomp < 0: raise ValueError("<m^2> calc failed.")

print(f"    <m>   = {avM_precomp:.3g}")
print(f"    <m^2> = {avM2_precomp:.3g}")
if avM_precomp > 0: print(f"    <m^2>/<m>^2 = {avM2_precomp/avM_precomp**2:.3g}")

# Precompute Disrupting Fraction Matrix (once, using QUAD)
N_grid = len(analytical_mass_grid)
disrupt_fraction_matrix_precomp = np.ones((N_grid, N_grid))
print(f"  Precomputing {N_grid}x{N_grid} disrupting fraction matrix (using quad)...")
    ...
    for j in range(N_grid):
        ...
        if mi > 0 and mj > 0:
            disrupt_fraction_matrix_precomp[i, j] = calculate_disrupting_fraction(
                mi, mj, psi_interp_callable, m_min_psi, m_max_psi, avM_precomp # Pass quad limits
            )
...
for f_val in f_pbh_values_plot:
    ...
    try:
        ...
        m_out, rate_mat, _, total_rate = calculate_merger_rate_matrix(
            ...
            m_min_psi=m_min_psi, m_max_psi=m_max_psi, # Pass grid limits
            avM=avM_precomp, avM2=avM2_precomp, # Pass precomputed moments
            disrupt_fraction_matrix=disrupt_fraction_matrix_precomp # Pass precomputed matrix
        )
    ...
    # f_end_time = time.time() # Reduce clutter
    # print(f"    Time for this f: {f_end_time - f_start_time:.2f} s") # Reduce clutter
...
# ... (Rest of the script: Convert arrays, Load Sim Results, Plotting - remains the same) ...
...
ax.set_xlim(1.e-4, 1.); ax.set_ylim(1e-6, 1e10)
...
plot_filename_pdf = "pbh_merger_rate_comparison_unequal_whiteBG_c11.pdf" # New name
